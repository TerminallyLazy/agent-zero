...
# Create a LangChain-compatible LLM wrapper for LiteLLM
from langchain_core.runnables import Runnable

class LiteLLMWrapper(Runnable):
    ...
    def invoke(self, input: Any, config: RunnableConfig | None = None, **kwargs: Any) -> str:
        ...
        if isinstance(input, str):
            ...
            # This is likely a formatted prompt from LangChain
            content = str(input)
        else:
            content = str(input)
        ...
        try:
            ...
            # Handle different response types from LiteLLM using defensive programming
            try:
                # For non-streaming responses, LiteLLM returns a ModelResponse object
                if hasattr(response, 'choices') and response.choices:
                    choice = response.choices[0]
                    # Check for message attribute (standard completion response)
                    if hasattr(choice, 'message'):
                        ...
                        if message:
                            ...
                            if content is not None:
                                return str(content)
                    # Check for text attribute (legacy format)
                ...
                print(f"Error parsing LiteLLM response: {parse_error}")
...
def eval_node(state):
    ...
    history.append({
        "iteration": state.get("iteration", 0),
        "domain"   : state["domain_info"].get("domain", "Unknown") if isinstance(state["domain_info"], dict) else json.loads(state["domain_info"])["domain"] if isinstance(state["domain_info"], str) else "Unknown",
        "scores"   : ev["scores"],
        "analysis_head": (
            state["analysis"].get("analysis", {}).get("descriptive", "")[:120]
            if isinstance(state["analysis"], dict) else
            (json.loads(state["analysis"])["analysis"]["descriptive"][:120])
        )
    })
