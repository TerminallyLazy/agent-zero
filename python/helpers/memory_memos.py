...
class MemOSClient:
    ...
    async def add_memory(self, text: str, metadata: Dict[str, Any], user_id: str = None) -> str:
        ...
        try:
            ...
            # Ensure cube is registered before adding memory
            await self.register_cube()
    ...
    async def search_memories(self, query: str, limit: int = 5, user_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Ensure cube is registered before searching
            await self.register_cube()
...
class MemOSMemory(Memory):
    ...
    async def search_similarity_threshold(
        self, query: str, memory_subdir: str = "", threshold: float = 0.1, count: int = 5, limit: int = None, filter: str = "", **kwargs
    ) -> list[Document]:
        """Search memories with similarity threshold."""
        # MemoryOS doesn't provide similarity scores, so we'll use regular search
        # Note: filter parameter is accepted for compatibility but not used by MemoryOS
        # Support both count and limit parameters for compatibility
        actual_limit = limit if limit is not None else count
        return await self.search(query, memory_subdir, actual_limit)
